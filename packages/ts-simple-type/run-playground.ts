...
import { isSimpleType, SimpleType } from './src/simple-type.js';
import { deserializeSimpleType, serializeSimpleType } from './src/transform/serialize-simple-type.js';
import { toSimpleType } from './src/transform/to-simple-type.js';
import { visitNodeComparisons } from './test/helpers/visit-type-comparisons.js';
...
function visitComparisons(sourceFile: SourceFile, program: Program) {
	...
	visitNodeComparisons(sourceFile, ({ line, nodeA, nodeB }) => {
		...
		// Run type checking
		if (!debug)
			console.time('type-checking-lazy');
		...
		if (!debug)
			console.timeEnd('type-checking-lazy');
