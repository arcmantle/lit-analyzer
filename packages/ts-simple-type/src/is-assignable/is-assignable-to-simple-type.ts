...
function isCacheableType(simpleType: SimpleType, options: IsAssignableToSimpleTypeInternalOptions): boolean {
	...
	return !('typeParameters' in simpleType)
		&& ![ 'GENERIC_ARGUMENTS', 'GENERIC_PARAMETER', 'PROMISE', 'LAZY' ].includes(simpleType.kind);
}
...
function isAssignableToSimpleTypeInternal(
	...
	// [typeA] (compare)
	case 'ENUM_MEMBER': {
		// A plain "number" is still in the domain of a numeric enum member, so it
		// stays assignable. A number *literal* is not: TypeScript only accepts it
		// when its value matches the member, so let it fall through to the literal
		// comparison below rather than short-circuiting to true here.
		if (resolveType(typeA.type, options.genericParameterMapA).kind === 'NUMBER_LITERAL' && typeB.kind === 'NUMBER') {
			if (typeB.name != null)
				return false;
...
function reduceIntersectionIfPossible(simpleType: SimpleTypeIntersection, parameterMap: Map<string, SimpleType>): SimpleType {
	...
	const typeKindMap: Map<SimpleTypeKind, SimpleType[]> = new Map();
	const primitiveSet: Set<SimpleTypeKind> = new Set();
	const primitiveLiteralSet: Map<SimpleTypeKind, unknown> = new Map();
	...
	if (typeKindMap.size === 1 && (typeKindMap.get('OBJECT')?.length || 0) > 1) {
		const members: Map<string, SimpleTypeMemberNamed> = new Map();
...
/**
 * The number of elements a tuple can hold, as an inclusive range.
 *
 * Optional members lower the minimum, and a rest member removes the maximum.
 */
function tupleArity(tuple: SimpleTypeTuple): { min: number; max: number; } {
	const required = tuple.members.filter(member => !member.optional).length;

	return {
		min: required,
		max: tuple.rest === true ? Number.POSITIVE_INFINITY : tuple.members.length,
	};
}

/**
 * Reports whether a type has no possible values, so nothing is assignable to it.
 *
 * TypeScript reduces an impossible intersection to `never`, and then rejects
 * `null` and `undefined` even when `strictNullChecks` is off. This mirrors only
 * the part of that reduction which is certain: one value cannot satisfy two
 * tuples whose lengths cannot overlap.
 *
 * General intersection inhabitance is deliberately not decided here. An
 * intersection of unrelated object types such as `{ a: string } & { b: number }`
 * is perfectly inhabited, so anything not proven empty is reported as inhabited.
 */
function isUninhabited(simpleType: SimpleType): boolean {
	if (simpleType.kind === 'NEVER')
		return true;


	if (simpleType.kind !== 'INTERSECTION')
		return false;


	const tuples = simpleType.types.filter((type): type is SimpleTypeTuple => type.kind === 'TUPLE');

	return tuples.some((tuple, index) => {
		const arity = tupleArity(tuple);

		return tuples.slice(index + 1).some(other => {
			const otherArity = tupleArity(other);

			return arity.min > otherArity.max || otherArity.min > arity.max;
		});
	});
}
