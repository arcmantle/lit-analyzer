			...
			return {
				...
				attributes: [
					...
					},
					{
						name: 'playsinline',
						description:
								'The playsinline attribute is a boolean attribute. If present, it serves as a hint to the user ' +
								'agent that the video ought to be displayed "inline" in the document by default, constrained to ' +
								'the element\'s playback area, instead of being displayed fullscreen or in an independent ' +
								'resizable window.',
						valueSet: 'v', // "v" is the undocumented boolean type
					},
				],
		...
		},
		{
			...
			attributes:  [
				...
				},
				{
					name: 'onslotchange',
					description:
						'The slotchange event is fired on an HTMLSlotElement instance (<slot> element) when the ' +
						"node(s) contained in that slot change.\n\nNote: the slotchange event doesn't fire if the " +
						'children of a slotted node change — only if you change (e.g. add or delete) the actual ' +
						'nodes themselves.',
				},
			],
		},
