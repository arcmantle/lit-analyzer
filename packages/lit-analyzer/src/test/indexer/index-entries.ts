...
import { getCurrentTsModule, TestContext, tsTest } from '../helpers/ts-test.js';
...
tsTest(
	'An entry is created for elements that are not defined with `customElements` if they are added to '
	+ '`HTMLElementTagNameMap` in TS.',
	t => {
		const { indexEntries, sourceFile } = getIndexEntries([
			{
				fileName: 'main.ts',
...
const assertIsAttrRefAndGetTarget = ({
	...
	name,
	kind,
}: { t: TestContext; entry: LitIndexEntry; name: string; kind: HtmlNodeAttrKind; }) => {
	...
	const { indexEntries } = getIndexEntries([
		{
			...
			text:     `
				...
				interface SomeElement {
					addEventListener<K extends keyof SomeElementEventMap>(
						type: K,
						listener: (this: SomeElement, ev: SomeElementEventMap[K]) => any,
					...
					removeEventListener<K extends keyof SomeElementEventMap>(
						type: K,
						listener: (this: SomeElement, ev: SomeElementEventMap[K]) => any,
